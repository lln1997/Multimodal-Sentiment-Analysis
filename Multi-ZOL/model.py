...
from layers import visual_aspect_attention, TFN, BiGRU_maxpooling, HAN, bidirectional_rnn, text_attention, visual_aspect_gate_unit
...
class VistaNet:
  ...
  def _init_word_encoder(self):
    with tf.variable_scope('word') as scope:
      ...
      self.word_outputs = tf.reduce_max(word_rnn_outputs, axis=1)
  ...
  def _init_sent_encoder(self):
    with tf.variable_scope('sentence') as scope:
      ...
      #self.sentence_outputs = BiGRU_maxpooling(
        #sent_input=sentence_rnn_outputs_1,
        #visual_input=self.images,
        #att_dim=self.att_dim,
      #)

      #self.sentence_outputs = TFN(
        #sent_input=sentence_rnn_outputs_1,
        #visual_input=self.images,
        #att_dim=self.att_dim,
      #)

      #self.sentence_outputs = HAN(
        #sent_input=sentence_rnn_outputs_1,
        #visual_input=self.images,
        #att_dim=self.att_dim,
        #sequence_lengths=self.document_lengths
