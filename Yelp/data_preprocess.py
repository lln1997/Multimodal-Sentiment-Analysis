...
# cities = ['Boston', 'Chicago', 'LosAngeles', 'NewYork', 'SanFrancisco']
cities = ['all_city']
...
def dump_cities_file():
  ...
  output_file = os.path.join(data_dir, 'test/{}_test.pickle'.format('all_city'))
  ...
  try:
    for city in cities:
      input_file = os.path.join(data_dir, 'test/{}_test.json'.format(city))
      for review in read_reviews(input_file):
        rating = review['Rating']
        photos = review['Photos']
        text = []
