...
#segmentor = Segmentor()
#segmentor.load(r'D:/Major/nlp/exercise/vagu_tf-main(muti-zol)/ltp_data_v3.4.0/cws.model')
...
#def word_tokenize(text):
  #sent = SentenceSplitter.split(text)
  #for s in sent:
...
def dump_file(input_file, output_file):
  ...
  try:
    for review in read_reviews(input_file):
      ...
      text = []
      for sent in SentenceSplitter.split(review['Text']):
        if len(sent) > 0:
          words = segmentor.segment(sent)
          text.append([vocab.get(i, UNK) for i in words])
...
def dump_cities_file():

  output_file = os.path.join(data_dir, 'test.pickle')
