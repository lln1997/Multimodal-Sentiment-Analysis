...
# 1.14
train_summary_writer = tf.summary.FileWriter('log' + '/train')
...
def main(_):
  ...
  with tf.Session(config=config) as sess:
    ...
    while epoch < FLAGS.num_epochs:
      ...
      print('=> Testing')
      test_loss, test_acc = evaluate(sess, data_reader.read_test_set(city='all_city'),
                                     model, loss, accuracy)
      print('test_acc: ', test_acc)
      result_file = open(
          os.path.join(FLAGS.log_dir, 'loss={:.4f},acc={:.4f}'.format(test_loss, test_acc)), 'w')
      result_file.write('loss={:.4f},acc={:.4f}\n'.format(test_loss, test_acc))
      result_file.flush()
      if test_acc > best_acc:
        best_acc = test_acc
        _loss = test_loss
    result_file1 = open(
        os.path.join(FLAGS.log_dir, 'best_acc={:.4f}'.format(best_acc)), 'w')
    result_file1.write('loss={:.4f},acc={:.4f}\n'.format(_loss, best_acc))
    result_file1.flush()
